    ...
    >
      <form className="space-y-10" onSubmit={handleSubmit}>
          ...
          <section className="rounded-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
            ...
            <div className="mt-8 space-y-6">
              <div className="space-y-2">
                ...
                <input className="w-full rounded-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" placeholder="University Name" value={settings.organizationName || ''} onChange={(event) => updateField('organizationName', event.target.value)} />
              </div>
              <div className="space-y-2">
                ...
                <textarea className="min-h-[140px] w-full rounded-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder="Ex: Computer Science" value={Array.isArray(settings.departments) ? settings.departments.join('\n') : settings.departments || ''} onChange={(event) => updateField('departments', event.target.value)} />
              </div>
              <div className="space-y-2">
                ...
                <textarea className="min-h-[100px] w-full rounded-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder="Ex: 2025" value={Array.isArray(settings.batches) ? settings.batches.join('\n') : settings.batches || ''} onChange={(event) => updateField('batches', event.target.value)} />
              </div>
            </div>
          </section>

          <section className="space-y-10">
            <div className="rounded-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
              <h2 className="text-2xl font-black tracking-tight dark:text-white flex items-center gap-3">
              ...
              <div className="mt-8 space-y-6">
                <div className="space-y-2">
                  ...
                  <input className="w-full rounded-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" placeholder="Ex: Spring 2026 Core" value={settings.activeCycle?.name || ''} onChange={(event) => updateField('activeCycle', { ...settings.activeCycle, name: event.target.value })} />
                </div>
            ...
            <div className="rounded-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
              ...
              <div className="mt-8 space-y-2">
                ...
                <input className="w-full rounded-sm border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" min="1" type="number" value={settings.projectReviewSlaDays || 7} onChange={(event) => updateField('projectReviewSlaDays', Number(event.target.value) || 7)} />
              </div>
            </div>
          </section>
        </div>
        ...
        <section className="rounded-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
            <div className="flex items-center gap-3">
              <div className="flex size-12 items-center justify-center rounded-sm bg-emerald-500/10 text-emerald-500">
            ...
            <button 
              className="rounded-sm border border-slate-200 dark:border-white/10 px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-white/5 transition-all shadow-sm active:scale-95" 
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {(settings.reviewRubric || []).map((item, index) => (
              <div key={`${item.key}-${index}`} className="relative group rounded-sm border border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 p-8 transition-all hover:border-primary/30">
                <div className="grid gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">System Key</label>
                       <input className="w-full rounded-sm border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-4 py-3 text-xs outline-none focus:border-primary dark:text-white font-mono" value={item.key || ''} onChange={(event) => updateRubric(index, 'key', event.target.value)} />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Max Score</label>
                       <input className="w-full rounded-sm border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-4 py-3 text-xs outline-none focus:border-primary dark:text-white" type="number" value={item.maxScore || 5} onChange={(event) => updateRubric(index, 'maxScore', Number(event.target.value) || 5)} />
                    </div>
                  </div>
                  <div className="space-y-2">
                     <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Display Label</label>
                     <input className="w-full rounded-sm border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-4 py-3 text-sm font-bold outline-none focus:border-primary dark:text-white" value={item.label || ''} onChange={(event) => updateRubric(index, 'label', event.target.value)} />
                  </div>
                  <div className="space-y-2">
                     <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Guideline Description</label>
                     <textarea className="min-h-[80px] w-full rounded-sm border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 px-4 py-3 text-xs outline-none focus:border-primary dark:text-white leading-relaxed" value={item.description || ''} onChange={(event) => updateRubric(index, 'description', event.target.value)} />
                  </div>
                </div>
                <button 
        ...
        <div className="flex items-center justify-end border-t border-slate-200 dark:border-white/5 pt-10">
           <button className="rounded-sm bg-primary px-10 py-5 text-sm font-black text-white shadow-xl shadow-primary/30 transition-all hover:scale-105 active:scale-95 disabled:opacity-70" disabled={saving} type="submit">
