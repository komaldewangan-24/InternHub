    ...
    >
      <form className="space-y-10" onSubmit={handleSubmit}>
        <div className="grid gap-10 lg:grid-cols-2">
          <section className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
            ...
            <div className="mt-8 space-y-6">
              <div className="space-y-2">
                ...
                <input className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" placeholder="University Name" value={settings.organizationName || ''} onChange={(event) => updateField('organizationName', event.target.value)} />
              </div>
              <div className="space-y-2">
                ...
                <textarea className="min-h-[140px] w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder="Ex: Computer Science" value={Array.isArray(settings.departments) ? settings.departments.join('\n') : settings.departments || ''} onChange={(event) => updateField('departments', event.target.value)} />
              </div>
              <div className="space-y-2">
                ...
                <textarea className="min-h-[100px] w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder="Ex: 2025" value={Array.isArray(settings.batches) ? settings.batches.join('\n') : settings.batches || ''} onChange={(event) => updateField('batches', event.target.value)} />
              </div>
            </div>
          </section>

          <section className="space-y-10">
            <div className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
              <h2 className="text-2xl font-black tracking-tight dark:text-white flex items-center gap-3">
              ...
              <div className="mt-8 space-y-6">
                <div className="space-y-2">
                  ...
                  <input className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" placeholder="Ex: Spring 2026 Core" value={settings.activeCycle?.name || ''} onChange={(event) => updateField('activeCycle', { ...settings.activeCycle, name: event.target.value })} />
                </div>
            ...
            <div className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
              ...
              <div className="mt-8 space-y-6">
                <div className="space-y-2">
                  ...
                  <input className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" min="1" type="number" value={settings.projectReviewSlaDays || 7} onChange={(event) => updateField('projectReviewSlaDays', Number(event.target.value) || 7)} />
                </div>
                <div className="flex items-center justify-between p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5">
                  <div>
                    <p className="text-sm font-black dark:text-white">Strict Company Verification</p>
                    <p className="text-[10px] font-medium text-slate-500 dark:text-slate-400">Force manual admin audit for all recruiter profiles.</p>
                  </div>
                  <button
                    type="button"
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${settings.companyVerificationEnabled ? 'bg-primary' : 'bg-slate-300 dark:bg-slate-700'}`}
                    onClick={() => updateField('companyVerificationEnabled', !settings.companyVerificationEnabled)}
                  >
                    <span className={`inline-block size-4 transform rounded-full bg-white transition-transform ${settings.companyVerificationEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>
              </div>
            </div>
        ...
        <section className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-2">
            <div className="space-y-2">
              ...
              <input className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" placeholder="General ATS Score" value={settings.resumeAtsCriteria?.title || ''} onChange={(event) => updateAtsCriteria('title', event.target.value)} />
            </div>
            <div className="space-y-2">
              ...
              <textarea className="min-h-[120px] w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder={'Contact\nSummary\nSkills\nEducation\nExperience\nProjects'} value={listToText(settings.resumeAtsCriteria?.requiredSections)} onChange={(event) => updateAtsCriteria('requiredSections', event.target.value)} />
            </div>
            <div className="space-y-2">
              ...
              <textarea className="min-h-[140px] w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder={'React\nNode.js\nMongoDB\nCommunication'} value={listToText(settings.resumeAtsCriteria?.requiredKeywords)} onChange={(event) => updateAtsCriteria('requiredKeywords', event.target.value)} />
            </div>
            <div className="space-y-2">
              ...
              <textarea className="min-h-[140px] w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white font-mono" placeholder={'GitHub\nLinkedIn\nImpact\nOptimized'} value={listToText(settings.resumeAtsCriteria?.preferredKeywords)} onChange={(event) => updateAtsCriteria('preferredKeywords', event.target.value)} />
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3 xl:grid-cols-6">
            {Object.keys(defaultAtsWeights).map((key) => (
              <div key={key} className="space-y-2">
                ...
                <input className="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-3 text-sm outline-none focus:border-primary dark:text-white" min="0" type="number" value={settings.resumeAtsCriteria?.weights?.[key] ?? defaultAtsWeights[key]} onChange={(event) => updateAtsWeight(key, event.target.value)} />
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 space-y-2">
            ...
            <textarea className="min-h-[90px] w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 px-4 py-4 text-sm outline-none focus:border-primary dark:text-white" placeholder="Optional guidance shown to students and admins." value={settings.resumeAtsCriteria?.notes || ''} onChange={(event) => updateAtsCriteria('notes', event.target.value)} />
          </div>
        </section>
        ...
        <section className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-10 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
            ...
            <button 
              className="rounded-xl border border-slate-200 dark:border-white/10 px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-white/5 transition-all shadow-sm active:scale-95" 
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {(settings.reviewRubric || []).map((item, index) => (
              <div key={`${item.key}-${index}`} className="relative group rounded-xl border border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 p-8 transition-all hover:border-primary/30">
        ...
        <div className="flex items-center justify-end border-t border-slate-200 dark:border-white/5 pt-10">
           <button className="rounded-xl bg-primary px-10 py-5 text-sm font-black text-white shadow-xl shadow-primary/30 transition-all hover:scale-105 active:scale-95 disabled:opacity-70" disabled={saving} type="submit">
