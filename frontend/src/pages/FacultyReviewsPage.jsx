            ...
            <div className="flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-8">
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="rounded-sm border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 p-6">
                    ...
                    <div className="space-y-3">
                      {(selectedProject.links || []).map((link) => (
                        <a key={link} className="flex items-center gap-2 truncate rounded-sm bg-white dark:bg-slate-950 border border-slate-100 dark:border-white/5 p-3 text-xs font-bold text-primary hover:underline hover:border-primary/30 transition-all" href={link} rel="noreferrer" target="_blank">
                ...
                <div className="space-y-4">
                  ...
                  <textarea className="min-h-[160px] w-full rounded-sm border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-6 py-4 text-sm outline-none focus:border-primary dark:text-white transition-all shadow-sm" placeholder="Detail the strengths or required improvements for this student submission..." value={feedback} onChange={(event) => setFeedback(event.target.value)} />
                </div>
              </div>

              <div className="p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 flex flex-wrap gap-4">
                <button className="flex-1 rounded-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 px-6 py-4 text-sm font-black text-slate-700 dark:text-slate-200 shadow-sm transition-all hover:border-slate-300 active:scale-95" disabled={saving} onClick={() => handleReview('commented')}>
                  Discard Review & Comment
                </button>
                <button className="flex-1 rounded-sm bg-amber-500 px-6 py-4 text-sm font-black text-white shadow-lg shadow-amber-500/20 transition-all hover:scale-105 active:scale-95" disabled={saving} onClick={() => handleReview('needs_resubmission')}>
                  Request Resubmission
                </button>
                <button className="flex-1 rounded-sm bg-primary px-6 py-4 text-sm font-black text-white shadow-lg shadow-primary/30 transition-all hover:scale-105 active:scale-95" disabled={saving} onClick={() => handleReview('approved')}>
                  Approve Project
                </button>
              </div>
            </div>
