...
const InternshipCard = ({ _id, title, company, location, salary, tags, match, onApply }) => (
  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-8 rounded-2xl flex-1 hover:border-indigo-500/50 transition-all shadow-sm hover:shadow-2xl group flex flex-col items-center text-center">
    ...
    <div className="w-full pt-6 border-t border-slate-50 dark:border-white/5 flex flex-col items-center gap-4 mt-auto">
      ...
      <button 
        onClick={() => onApply(_id)}
        className="text-[11px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest font-poppins hover:scale-105 transition-transform flex items-center gap-2"
      >
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        setPageLoading(true);
        const { data: res } = await studentAPI.getDashboard();
        setData(res.data);
      } finally {
  ...
  if (loading || pageLoading || !data) return <LoadingState label="Hydrating dashboard parameters..." />;

  const { stats, readiness, recommendations, skills, upcomingInterviews } = data;
      ...
      <div className="max-w-[1440px] mx-auto pt-0 mb-4 overflow-hidden relative">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-100 dark:border-white/5 pb-6">
          <div className="space-y-2">
            ...
            <h2 className="text-slate-500 dark:text-slate-400 font-medium text-md lg:text-lg font-roboto">
              You’re making great progress. Let’s keep going!
            </h2>
          </div>
          <div className="flex flex-wrap gap-4 shrink-0">
            <button onClick={() => navigate('/student/resume')} className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-white dark:bg-white/5 text-slate-600 dark:text-slate-300 font-bold text-[12px] uppercase tracking-widest transition-all border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md font-poppins group">
              <span className="material-symbols-outlined text-[20px] group-hover:-translate-y-1 transition-transform">download</span>
              Resume
            </button>
            <button onClick={() => navigate('/internships')} className="flex items-center gap-2 px-8 py-3 rounded-2xl bg-blue-600 text-white font-bold text-[12px] uppercase tracking-widest shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all border border-blue-500 font-poppins group">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-8 shadow-sm relative overflow-hidden group">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium font-roboto leading-none max-w-xl">
                You’re {readiness.profileCompletion}% placement-ready. Let’s push it to 100% 🚀
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            {readiness.flags.length === 0 ? (
               <span className="flex items-center gap-2 px-6 py-2 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-xl text-[10px] font-black uppercase tracking-widest font-poppins border border-emerald-100 dark:border-emerald-500/20">
                <span className="material-symbols-outlined text-[16px]">verified</span> Profile Verified
              </span>
            ) : (
              readiness.flags.map((flag, idx) => (
                <span key={idx} className="flex items-center gap-2 px-6 py-2 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 rounded-xl text-[10px] font-black uppercase tracking-widest font-poppins border border-amber-100 dark:border-amber-500/20">
                  <span className="material-symbols-outlined text-[16px]">warning</span> {flag.replace(/_/g, ' ')}
                </span>
              ))
            )}
            <button onClick={() => navigate('/student/profile')} className="flex items-center gap-2 px-6 py-2 bg-white dark:bg-white/5 border-2 border-dashed border-primary/30 text-primary rounded-xl text-[10px] font-black uppercase tracking-widest font-poppins hover:bg-primary hover:text-white transition-all">
              <span className="material-symbols-outlined text-[16px]">add_circle</span> Complete Profile
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { 
                  ...
                  value: user.profile?.resumeUrl ? 'Verified' : 'Missing', 
                  icon: 'visibility', 
                  description: user.profile?.resumeUrl ? 'Your resume is visible to recruiters' : 'Upload your resume to start applying',
                  trend: user.profile?.resumeUrl ? 'Active' : 'Missing',
                  color: user.profile?.resumeUrl ? 'emerald' : 'rose'
                },
                { 
                  label: 'Career Readiness', 
                  value: `${readiness.score}%`, 
                  icon: 'rocket_launch', 
                  description: 'Based on projects, skills and profile strength',
                  trend: readiness.score > 70 ? 'Ready' : 'Level up',
                  color: readiness.score > 70 ? 'emerald' : 'amber'
                },
                { 
                  label: 'Applications', 
                  ...
                  <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-6 rounded-2xl shadow-sm relative overflow-hidden group hover:border-slate-300 dark:hover:border-white/20 transition-all flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-6">
                        ...
                        {stat.trend && (
                          <div className={`text-[9px] font-black ${stat.color === 'rose' ? 'text-rose-500 bg-rose-50' : 'text-emerald-500 bg-emerald-50'} dark:bg-opacity-10 px-2 py-1 rounded-sm uppercase tracking-widest font-poppins`}>
                            {stat.trend}
                          </div>
                        )}
                      </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-8 shadow-sm space-y-8">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-6">
                <h2 className="text-xl font-bold text-[#003366] dark:text-white font-poppins tracking-tighter uppercase">Recommendations</h2>
                <button onClick={() => navigate('/internships')} className="text-[11px] font-black text-blue-500 uppercase tracking-widest hover:opacity-70 transition-all font-poppins flex items-center gap-2">View All <span className="material-symbols-outlined text-sm">open_in_new</span></button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {recommendations.map((item, idx) => (
                  <InternshipCard 
                    key={idx} 
                    {...item} 
                    onApply={(id) => navigate(`/internships/${id}`)} 
                  />
                ))}
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-[#003366] border border-slate-200 dark:border-white/5 rounded-2xl p-6 text-slate-800 dark:text-white shadow-sm dark:shadow-2xl relative overflow-hidden group">
              ...
              <h3 className="text-xl font-bold font-poppins mb-8 uppercase tracking-tighter text-[#003366] dark:text-white">Active Pipelines</h3>
              <div className="flex justify-center mb-8">
                <div className="size-40 rounded-full border-[10px] border-slate-100 dark:border-white/5 flex items-center justify-center relative shadow-sm dark:shadow-2xl">
                  <div className="absolute inset-0 rounded-full border-[10px] border-primary border-t-transparent animate-[spin_10s_linear_infinite]" />
                  <div className="text-center">
                    <span className="text-4xl font-black font-poppins tracking-tighter text-[#003366] dark:text-white">{stats.applications}</span>
                    <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-slate-400 dark:text-white/50 font-poppins mt-1">Ongoing</p>
                  </div>
                </div>
              </div>
              <div className="space-y-3">
                {[
                  { label: 'Project Backlog', count: stats.underReview, color: 'bg-amber-500' },
                  { label: 'Project Approved', count: stats.approved, color: 'bg-emerald-500' },
                  { label: 'Live Applications', count: stats.applications, color: 'bg-blue-500' }
                ].map((status, i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:bg-white dark:hover:bg-white/10 transition-all group/status">
                    <div className="flex items-center gap-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-6 shadow-sm relative overflow-hidden group">
              <div className="flex items-center justify-between mb-8 border-b border-slate-100 dark:border-white/5 pb-6">
                 <h3 className="text-xl font-bold text-[#003366] dark:text-white font-poppins uppercase tracking-tighter">Timeline</h3>
              ...
              <div className="space-y-6">
                {upcomingInterviews.map((interview, idx) => (
                  <div key={idx} className="relative pl-6 border-l-2 border-primary/20 hover:border-primary transition-all">
                    <p className="text-[11px] font-black text-primary uppercase tracking-[0.4em] font-poppins mb-3">
                       {new Date(interview.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }).toUpperCase()} • {new Date(interview.date).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                    </p>
                    <h4 className="text-[16px] font-bold text-slate-800 dark:text-white font-poppins mb-2 uppercase tracking-tight leading-tight">{interview.title}</h4>
                    <p className="text-[13px] text-slate-500 dark:text-slate-400 font-medium font-roboto leading-relaxed mb-4">{interview.company}</p>
                  </div>
                ))}
              </div>
              <button onClick={() => navigate('/applications')} className="w-full mt-12 py-5 rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/10 text-slate-400 text-[11px] font-black uppercase tracking-widest hover:border-primary hover:text-primary hover:bg-slate-50 dark:hover:bg-white/5 transition-all font-poppins">Manage Applications</button>
            </div>
          </div>
        </div>
      </div>
